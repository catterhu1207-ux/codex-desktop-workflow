"""Run the release contracts against an explicitly supplied real 6896 artifact."""
from pathlib import Path
import os
import unittest

import hotfix_builder as builder
import hotfix_profile_26917_6896 as profile
from frontend_feature_contracts import ContractError
from frontend_contract_26917_6896 import (
...
from frontend_work_contract_26917_6896 import PICKER_PATH, run as run_work


class ExactProfile6896(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        value = os.environ.get("CODEX_WORKFLOW_TEST_ASAR_6896")
        if not value:
            raise unittest.SkipTest("Set CODEX_WORKFLOW_TEST_ASAR_6896 to the exact built artifact")
        path = Path(value)
        if not path.is_file():
    ...
    def test_route_cannot_point_to_missing_picker(self):
        route = b"import(`./composer-project-picker-content-531ef99549e8.js`)"
