...
def _scenario_source() -> str:
    local = Path(__file__).with_name("frontend_scenarios_26915_4065.js")
    if local.is_file():
    ...
    return (
        ...
        .joinpath("data", "frontend_scenarios_26915_4065.js")
        .read_text(encoding="utf-8")
    )


FUNCTIONS = ('rI', 'Poi', 'iZs', 'aZs', 'Rxn', 'fPt', 'm6', 'l7s', 'qZx', 'T9r', 'Woi', 'Wsi', 'eOi', 'tOi', 'Joi', 'tni', 'NF', 'qsi', 'Jsi', 'Usi', 'lai', 'fai', 'oBt', 'Poo', 'bSc', 'MTc', 'Sti', 'qZp')
ROUTES = {'plan_pending_detection': (('entry_path', b'pt=bf(bri,n),mt=bf(u8n,n)'), ('entry_path', b'u8n=zE((e,t)=>n3n(e,t(gtn)))')), 'remote_project_label': (('entry_path', b'return Woi(e(tI),n,r)'), ('entry_path', b'xgc as ph,'), ('secondary_entry_path', b'ph as eve,')), 'work_remote_project_picker': (('entry_path', b'Cti as dSt,'), ('secondary_entry_path', b'dSt as _y,'), ('secondary_entry_path', b'jsx)($7,{menuOpen:nn,onMenuOpenChange:rn,projectId:ge??null,shortcut:an,subtleHover:!0,onProjectChange:void 0}'), ('secondary_entry_path', b'jsx)(D0e,{})'), ('secondary_entry_path', b'import(`./composer-project-picker-content-cee23446c3c9.js`)'), ('secondary_entry_path', b'P5e as Z,'), ('secondary_entry_path', b'$5e as F,')), 'new_chat_file_drop': (('secondary_entry_path', b'handleDrop:Vc,handlePaste:Hc}=DGe({'), ('secondary_entry_path', b'BY as os,'), ('entry_path', b'Ioo as BY,'))}
...
def run_semantics(initial, primary, node='node'):
    ...
    cp = subprocess.run([node, '-'], input=_with_module_stubs(script, functions), text=True, capture_output=True, timeout=30)
    if cp.returncode:
        raise ContractError('26.915 actual function test: ' + cp.stderr[-3000:])
...
def validate(source_asar, portable_asar, node='node'):
    ...
    from frontend_work_contract_26915_4065 import run as run_work, PICKER_PATH, PICKER_SHA256
    ...
    fnmap = {'priority_filter_recency_sorting': ['rI'], 'priority_filter_live_resort': ['rI'], 'priority_filter_pinned_recency_sorting': ['rI'], 'priority_filter_hold_membership': ['iZs', 'aZs'], 'priority_click_hold': ['bSc'], 'priority_identity_migration': ['Poi'], 'priority_project_context_subtitle': ['Joi'], 'plan_pending_detection': ['qZp'], 'plan_pending_yellow_indicator': ['m6', 'l7s'], 'plan_pending_unread_indicator': ['qZp', 'm6', 'l7s'], 'attention_highlight_color_semantics': ['qZp', 'm6', 'l7s'], 'project_sorting': ['Wsi', 'rI'], 'active_priority_sort': ['eOi', 'tOi'], 'automation_priority_gate': ['aZs', 'iZs'], 'pinned_priority_sync': ['Poi'], 'new_chat_file_drop': ['kGe', 'K5e', 'Poo', 'MTc', 'Ioo'], 'resume_history_on_demand': ['Rxn'], 'paginated_tail_retention': ['fPt'], 'idle_history_eviction': ['oBt'], 'remote_project_label': ['qZx', 'T9r', 'Woi'], 'work_remote_project_picker': ['Cti', '$7', 'i7e', 'B$', 'O0e', 'u9e', 'I', 'P5e', 'e7e', 'Aka']}
    contracts = {}
